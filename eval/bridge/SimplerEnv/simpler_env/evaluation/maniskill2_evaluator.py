...
def run_maniskill2_eval_single_episode(
    ...
    if os.path.exists(video_dir):
        for mp4 in pathlib.Path(video_dir).iterdir():
            if not mp4.is_file() or not mp4.suffix == ".mp4":
                continue
            if f"obj_episode_{obj_episode_id}_" in mp4.name:
                print(f"Skipping {mp4.parents[1].name}/{obj_episode_id}, already done.")
    ...
    while not (predicted_terminated or truncated):
        ...
        gripper_proprio = (gripper_proprio - 0.015) / (0.037 - 0.015)
        action = model.step(image, task_description, ee_pose_proprio, gripper_proprio)
    ...
    model.visualize_epoch(images, save_path=action_path)
