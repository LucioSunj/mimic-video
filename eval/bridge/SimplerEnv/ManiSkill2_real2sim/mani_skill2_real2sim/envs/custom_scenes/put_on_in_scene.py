...
class PutOnInSceneEnv(MoveNearInSceneEnv):
    ...
    def evaluate(self, success_require_src_completely_on_target=True, z_flag_required_offset=0.02, **kwargs):
        ...
        xy_flag = np.linalg.norm(offset[:2]) <= np.linalg.norm(tgt_obj_half_length_bbox[:2]) + 0.003
        z_flag = (offset[2] > 0) and (
            offset[2] - tgt_obj_half_length_bbox[2] - src_obj_half_length_bbox[2] <= z_flag_required_offset
        )
...
class StackGreenCubeOnYellowCubeBakedTexInScene(StackGreenCubeOnYellowCubeInScene):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(source_obj_name=source_obj_name, target_obj_name=target_obj_name, **kwargs)
...
class PutEggplantInBasketScene(PutOnBridgeInSceneEnv):
    ...
    def evaluate(self, *args, **kwargs):
        return super().evaluate(
            success_require_src_completely_on_target=False, z_flag_required_offset=0.06, *args, **kwargs
        )
